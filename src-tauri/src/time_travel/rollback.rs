...
pub fn generate_rollback_statements(entries: &[ChangelogEntry], driver_id: &str) -> RollbackResult {
            ...
            ChangeOperation::Update => {
                match &entry.before {
                    Some(before) => {
                        ...
                        // Restore changed columns only; if none were tracked, restore every non-PK column.
            ...
            ChangeOperation::Delete => {
                match &entry.before {
                    Some(before) => {
                        ...
                        // Warn for binary columns that format_literal rejected.
...
fn format_table_ref(
    ...
    if let Some(schema) = schema {
        ...
        // MySQL/MariaDB have no schema layer; qualify with the database instead.
        format!(
            "{}.{}",
    ...
    match value {
        ...
        serde_json::Value::String(s) => {
            if s.starts_with("<binary ") && s.ends_with(" bytes>") {
                None // binary blobs are not safely round-trippable as text literals
            } else {
