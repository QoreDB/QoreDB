...
use crate::engine::sql_generator::{
    generate_migration_script, MigrationScript, SandboxChangeDto, SandboxChangeType,
};
...
pub struct MigrationScriptResponse {
    pub success: bool,
    pub script: Option<MigrationScript>,
...
/// Generate migration SQL from sandbox changes
///
/// This command takes a list of sandbox changes and generates a SQL migration script
/// appropriate for the driver being used.
#[tauri::command]
...
/// Apply sandbox changes to the database
///
/// This command applies the sandbox changes to the actual database.
/// It can optionally wrap all changes in a transaction.
#[tauri::command]
#[instrument(skip(state, changes), fields(session_id = %session_id))]
    ...
    for (idx, change) in changes.iter().enumerate() {
        ...
        match result {
            ...
            Err(e) => {
                ...
                // If using transaction, rollback on first error
                if use_transaction && supports_tx {
                    ...
                    return Ok(ApplySandboxResponse {
                        success: false,
                        applied_count,
                        error: Some(format!("Change {} failed: {}. Transaction rolled back.", idx + 1, e)),
                        failed_changes,
                    });
                }
            }
