...
// Sandbox state and preferences, persisted to localStorage.
...
export function addSandboxChange(change: Omit<SandboxChange, 'id' | 'timestamp'>): SandboxChange {
  ...
  // Updates/deletes that target a row inserted in the same sandbox are folded
  // into the original insert rather than stacked as separate changes.
  if (change.type === 'update' || change.type === 'delete') {
    ...
    if (existingInsertIndex >= 0) {
      if (change.type === 'delete') {
        ...
        return newChange; // ghost change, returned for API consistency
      } else {
    ...
    // Coalesce consecutive updates to the same row into one change.
...
export function importChanges(sessionId: string, changes: SandboxChange[]): void {
  ...
  // Reassign IDs to avoid collisions with existing changes.
