 ...
 * cannot recover from. Best-effort, not exhaustive.
 */
export function isSystemReserved(chord: KeyChord): boolean {
  ...
  // Cmd/Ctrl + Tab — task switcher
  if (mods.has('mod') && key === 'Tab') return true;
  // Cmd+Q / Ctrl+Q — quit on macOS, sometimes on Linux
  if (mods.has('mod') && !mods.has('shift') && !mods.has('alt') && key === 'q') return true;
