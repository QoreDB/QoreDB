...
- **Mitigation**:
  ...
  - **Agent previews**: native `preview_table` calls consume the same per-session query budget as `run_query` and pass through the safety interceptor. Successful previews (including cache hits) and execution errors are audited with their agent source.
  ...
  - **Column masking (Pro)**: masking rules are bound to the session when a saved connection opens it and applied before rows leave QoreDB (streamed and buffered results, table pages, exports, federated sources before DuckDB, Data API, mutations, AI and contract samples, full-text hits, MCP and CLI). MCP refreshes the rules before reusing a cached session and closes it if exposure is revoked or its saved connection disappears. On a connection with masking, an agent query may only name a masked column as a bare projection: aliases, expressions, filters, sorts, join conditions, set operations and whole-row serialisation are refused. Positional column alias lists on CTEs and table references are also refused, including when the inner projection uses `*`.
  - **Masked edits and exports**: row/document editors submit only changed fields. Desktop structured updates reject masked fields and masked primary keys in the backend, including nested document keys and dotted paths. Replacing a whole object containing masked children is refused; editing a different top-level field remains possible. Parquet represents masked columns as text while preserving nulls.
- **Current limitation**:
  - Masking is hygiene against accidental exposure, not an access control for someone writing SQL in the editor: free queries match rules by column name only, so `SELECT email AS e` shows the value to the user. Database grants and views remain the boundary. Keyset pagination cursors carry the values of their ordering columns, a masked primary key included.
  ...
  - Query results are returned to the agent and may be sent to a remote model by the client; only columns covered by masking rules are hidden.
