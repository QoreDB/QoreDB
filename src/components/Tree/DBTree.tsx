...
export function DBTree({
	...
	const [expandedNamespace, setExpandedNamespace] = useState<Namespace | null>(
		null,
	);
	...
	const [createTableNamespace, setCreateTableNamespace] =
		useState<Namespace | null>(null);
	const [deleteModalOpen, setDeleteModalOpen] = useState(false);
	const [deleteTargetNamespace, setDeleteTargetNamespace] =
		useState<Namespace | null>(null);
	const [search, setSearch] = useState("");
	const [searchValue, setSearchValue] = useState("");
	const [collapsedActiveNsKey, setCollapsedActiveNsKey] = useState<
		string | null
	>(null);
	...
	const canLoadMore =
		collections.length > 0 && collections.length < collectionsTotal;
		...
		<div className="flex flex-col text-sm">
			...
			{namespaces.map((ns) => {
							...
							<div className="flex flex-col ml-2 pl-2 border-l border-border mt-0.5 space-y-0.5">
								...
								{(() => {
									const tables = collections.filter(
										(c) =>
											c.collection_type === "Table" || c.collection_type === "Collection",
									);
                ...
                type='button'
								...
								{/* Functions Section */}
								{(() => {
									const functions = routines.filter(
										(r) => r.routine_type === "Function",
									);
