...
impl AuthPlugin {
    pub(super) async fn scramble(
        ...
        if let Some(response) = passwordless_auth_response(password) {
            return Ok(response);
        }
...
fn passwordless_auth_response(password: &str) -> Option<Vec<u8>> {
    password.is_empty().then(Vec::new)
}
...
#[cfg(test)]
mod tests {
    use super::{passwordless_auth_response, scramble_sha1, scramble_sha256};
    use bytes::buf::Buf;
    use bytes::Bytes;

    fn nonce() -> bytes::buf::Chain<Bytes, Bytes> {
        Bytes::from_static(b"12345678").chain(Bytes::from_static(b"abcdefghijkl"))
    }

    #[test]
    fn empty_password_uses_an_empty_auth_response() {
        assert_eq!(passwordless_auth_response(""), Some(Vec::new()));
    }

    #[test]
    fn non_empty_passwords_are_still_scrambled() {
        assert_eq!(passwordless_auth_response("secret"), None);
        assert_eq!(scramble_sha1("secret", &nonce()).len(), 20);
        assert_eq!(scramble_sha256("secret", &nonce()).len(), 32);
    }
}
