...
//! TimescaleDB Driver
//!
//! TimescaleDB is a PostgreSQL extension for time-series data. This driver
//! is a thin wrapper over the shared `pg_compat` helpers exposing a distinct
//! `driver_id` for telemetry and UI branding. Future work may surface
//! hypertables and continuous aggregates as first-class objects in the tree.
...
pub struct TimescaleDbDriver {
    sessions: SessionMap,
}

impl TimescaleDbDriver {
    pub fn new() -> Self {
        Self {
...
impl Default for TimescaleDbDriver {
    fn default() -> Self {
        Self::new()
    }
}
...
#[async_trait]
impl DataEngine for TimescaleDbDriver {
    ...
    async fn create_database(
        ...
        pg_compat::create_schema(&self.sessions, session, name, "TimescaleDB").await
    }
    ...
    async fn truncate_all(
        ...
        pg_compat::truncate_all(&self.sessions, session, namespace, "TimescaleDB").await
    }
...
mod tests {
    ...
    #[test]
    fn timescaledb_driver_identity() {
        let d = TimescaleDbDriver::new();
        assert_eq!(d.driver_id(), "timescaledb");
        assert_eq!(d.driver_name(), "TimescaleDB");
    }
