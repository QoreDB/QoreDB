...
**Parcours navigateur VALIDÉ ✅** (Chrome headless, Postgres live) : register 1er admin → login → connexion → éditeur → `SELECT` → grille de résultats (SSE), en HTTP et en HTTPS. 3 corrections au passage : `get_driver_info` bridgé (sans lui `driverCapabilities` restait vide en web → résultats jamais rendus), `webExecuteQuery` assemble lui-même le résultat quand aucun handler de streaming n'est fourni (drivers non-streaming), et `ServeDir::fallback` au lieu de `not_found_service` (le SPA partait avec un statut 404).
**TLS ✅** : `QORE_SERVER_TLS_CERT`/`QORE_SERVER_TLS_KEY` (PEM, rustls/aws-lc-rs, zéro OpenSSL) ; config partielle → exit au boot ; sans les deux vars → HTTP (reverse proxy). Documenté dans le README de la crate.
**Reste (durcissement)** : surface `dialog`/`fs` web ; commandes encore non bridgées (erreur call-time tolérée par l'UI : `list_plugins`/`get_plugin_*`, `ai_get_provider_status`, `get_license_status`, `list_federation_sources`).
...
| **R2** | `qore-server` **v0.1** self-hosted : web servi par le serveur + register/login + RBAC local + TLS + Docker | Enterprise / BUSL-1.1 | validation navigateur ✅ + TLS ✅ | ✅ **Prête** (reste un re-test Docker avant tag) |
| **R3** | `qore-server` **v0.2** : SSO — OIDC validé sur Keycloak réel + SAML | Enterprise / BUSL-1.1 | abstraction `IdentityProvider` avant SAML | 🟡 OIDC **validé sur Keycloak réel ✅** ; reste SAML |
...
**Chemin critique** : R2 est la première release monétisable du control plane. Ses deux bloquants (**validation bout-en-bout en navigateur** et **option TLS**) sont levés ✅ — reste un smoke test du packaging Docker avant de tagger. R1 peut sortir à tout moment. Prochain front : R3 (callback OIDC sur Keycloak réel, puis abstraction `IdentityProvider` avant SAML).
