...
- **Mitigation**:
  ...
  - **Column masking (Pro)**: masking rules are bound to the session when a saved connection opens it and applied before rows leave QoreDB (streamed and buffered results, table pages, exports, federated sources before DuckDB, Data API, mutations, AI and contract samples, full-text hits, MCP and CLI). On a connection with masking, an agent query may only name a masked column as a bare projection: aliases, expressions, filters, sorts, join conditions, set operations and whole-row serialisation are refused.
- **Current limitation**:
